...
export class NotificationsController {
  ...
  async sendEmail(@Body() emailDto: SendEmailDto, @Req() request: any) {
    const result = await this.notificationsService.sendEmailNotification(emailDto);
  ...
  @Post('email/bulk')
  @ApiOperation({ summary: 'Send bulk email notifications (Admin/SuperAdmin only)' })
  @ApiResponse({ 
    status: HttpStatus.OK, 
  ...
  @UseGuards(RolesGuard)
  @Roles('ADMIN', 'SUPERADMIN')
  async sendBulkEmails(@Body() bulkEmailDto: BulkEmailDto, @Req() request: any) {
    const result = await this.notificationsService.sendBulkEmailNotifications(bulkEmailDto.emails);
  ...
  async sendWelcomeEmail(
    ...
    const result = await this.notificationsService.sendWelcomeEmail(body.email, body.name);
  ...
  @Post('email/template')
  @ApiOperation({ summary: 'Send template-based email using Mailgun templates' })
  @ApiResponse({ 
    status: HttpStatus.OK, 
