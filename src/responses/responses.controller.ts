...
export class ResponsesController {
  ...
  async createOrResumeSession(
    ...
    const session = await this.responsesService.createOrResumeSession(
      req.user.userId,
      createSessionDto,
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'Response auto-saved successfully',
  ...
  async autoSaveResponse(
    ...
    const result = await this.responsesService.autoSaveResponse(
      sessionId,
      autoSaveDto,
    );
    return this.responseService.success(
      result,
      'Response auto-saved successfully',
    );
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Responses auto-saved successfully',
    type: BatchAutoSaveResultDto,
  })
  async batchAutoSave(
    @Param('sessionId', ParseIntPipe) sessionId: number,
    @Body() batchAutoSaveDto: BatchAutoSaveDto,
  ) {
    const result = await this.responsesService.batchAutoSave(
      sessionId,
      batchAutoSaveDto,
    );
    return this.responseService.success(
      result,
      'Responses auto-saved successfully',
    );
  }
  ...
  @ApiResponse({
    ...
    type: ProgressResultDto,
  })
  async updatePosition(
    @Param('sessionId', ParseIntPipe) sessionId: number,
    @Body() updatePositionDto: UpdatePositionDto,
  ) {
    const result = await this.responsesService.updatePosition(
      sessionId,
      updatePositionDto,
    );
    return this.responseService.success(
