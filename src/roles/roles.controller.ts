...
export class RolesController {
  ...
  @Post('assign')
  @ApiOperation({
    summary: 'Assign role to user by role name (Admin/SuperAdmin only)',
  })
  @ApiResponse({
    status: 200,
  ...
  @Post('assign-by-id')
  @ApiOperation({
    summary: 'Assign role to user by role ID (Admin/SuperAdmin only)',
  })
  @ApiResponse({
    status: 200,
    description: 'Role assigned successfully',
  ...
  assignRoleById(@Body() assignRoleByIdDto: AssignRoleByIdDto) {
    return this.rolesService.assignRoleToUserById(
      assignRoleByIdDto.userId,
      assignRoleByIdDto.roleId,
    );
  }

  @Delete('remove')
  @ApiOperation({
    summary: 'Remove role from user by role name (Admin/SuperAdmin only)',
  })
  @ApiResponse({ status: 200, description: 'Role removed successfully' })
  @ApiResponse({
    status: 404,
    description: 'Role not found or user does not have this role',
  })
  @Roles('ADMIN', 'SUPERADMIN')
  @UseGuards(RolesGuard)
  ...
  @Delete('remove-by-id')
  @ApiOperation({
    summary: 'Remove role from user by role ID (Admin/SuperAdmin only)',
  })
  @ApiResponse({ status: 200, description: 'Role removed successfully' })
  @ApiResponse({
    status: 404,
  ...
  removeRoleById(@Body() assignRoleByIdDto: AssignRoleByIdDto) {
    return this.rolesService.removeRoleFromUserById(
      assignRoleByIdDto.userId,
      assignRoleByIdDto.roleId,
    );
  }
